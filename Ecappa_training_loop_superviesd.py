...
def main(run=None, config_file='/app/config.yaml'):
    ...
    for epoch in range(config['adaptation']['num_epochs']):
        ...
        train_epoch(speaker_brain, train_loader, label_map, config, epoch)
...
def train_epoch(speaker_brain, train_loader, label_map, config, epoch):
...
def evaluate_model(speaker_brain, dataloader, label_map, config, epoch, train=False):
    ...
    with torch.no_grad():
        for paths, numeric_labels in dataloader:
            # Avoid UserWarning by ensuring numeric_labels is a list or np array
            ...
            # Classifier logits for accuracy (optional if you only need EER)
            logits = speaker_brain.hparams.classifier(embeddings, labels_tensor)
            preds = torch.argmax(logits, dim=1)

            # Efficient pair construction (within-batch only or sampled)
