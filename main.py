...
def single_family_pipeline(config, labels_csv_file_path, 
    ...
    assert num_of_speakers > 0, "Number of speakers must be a positive integer >0"
    ...
    assert len(centroids) == num_of_speakers, "Number of centroids must be equal to the number of speakers"
    ...
    assert family_test_group.shape[0] > 0, "No test samples found for family_id {family_id}. Skipping...\n"
    test_sample_num = 0
    for _, row in family_test_group.iterrows():
        ...
        assert test_emb is not None, "Test embedding must not be None"
        if test_emb is not None: #redandent
            test_sample_num += 1
...
if __name__ == '__main__':
    ...
    #להראות אדפטציה
    ##להראות שיפור עם התקדמות האיפוקים
    ###בעזרת גרפים
    #להראות מה חקרנו
    ##לשפר את האימבדינג באמצעות הקליסטור
  
    #להגדיל את המשפחות          #####           לאמן על משפחות ולבחון שיפור על הכלל
    #
    #              יכול להקשות על הקלטרים                               
    #           
    #
    #